...
#define FIELD_WIDTH (fontx + 1)
#define FIELD_HEIGHT (fonty + 1)

#define COLUMNS 17
#define ROWS 7
#define BOMB_COUNT ((COLUMNS * ROWS) / 6)

#define WIDTH (COLUMNS + 2)
#define HEIGHT (ROWS + 2)

#define offset_x 12  //((LCDWIDTH - (COLUMNS) * FIELD_WIDTH) / 2) + 17;
#define offset_y (LCDHEIGHT - (ROWS)*FIELD_HEIGHT) / 2
...
void draw_board() {
  ...
  if (game_state == LOST) {
    gb.display.drawChar(3,
                        2,
                        0x01,
                        1);
  } else if (ADown) {
    if (board[cursor.x + 1][cursor.y + 1].state == COVERED) {
      gb.display.drawBitmap(3,
                            2,
                            neutral,
                            0,
                            0);
...
void loop() {
  if (gb.update()) {
    ...
    if (game_state == LOST) {
      if (gb.buttons.pressed(BTN_A)) new_game();
    }
  }
}